...
class DepthWiseAttention(UNetModel):
    ...
    # >>> [CorrAdapter MODIFIED BEGIN: DepthWiseAttention cache-aware forward]
    # Original SyncDreamer signature:
    # def forward(self, x, timesteps=None, context=None, source_dict=None, **kwargs):
    def forward(self, x, timesteps=None, context=None, source_dict=None,
                diffusion_index=-1, memory_corrs=None, memory_qks=None, **kwargs):
        ...
        for index, module in enumerate(self.input_blocks):
            # TimestepEmbedSequential now returns caches.
        ...
        for index, module in enumerate(self.output_blocks):
            ...
            # Original SyncDreamer used:
            # h = module(h, emb, context)
            # CorrAdapter is attached only to the last three output blocks,
            # which are the high-resolution self-attention blocks used for
            # native correspondence aggregation.
            corr_blocks = [9, 10, 11]
            if index in corr_blocks:
                corr_index = index - corr_blocks[0]
                block_corrs = memory_corrs[corr_index] if len(memory_corrs) > corr_index else []
                block_qks = memory_qks[corr_index] if len(memory_qks) > corr_index else []
                h, memory_corrs_update, memory_qks_update = module(
                    h,
                    emb,
                    context,
                    index=diffusion_index,
                    memory_corrs=block_corrs,
                    memory_qks=block_qks,
                )
                if len(memory_corrs) > corr_index:
                    memory_corrs[corr_index] = memory_corrs_update
                else:
                    memory_corrs.append(memory_corrs_update)
                if len(memory_qks) > corr_index:
                    memory_qks[corr_index] = memory_qks_update
                else:
                    memory_qks.append(memory_qks_update)
            else:
                h, _, _ = module(h, emb, context, index=diffusion_index)
        ...
        # return self.out(h)
        return self.out(h), memory_corrs, memory_qks
    # <<< [CorrAdapter MODIFIED END: DepthWiseAttention cache-aware forward]
