...
class UNetWrapper(nn.Module):
    ...
    def forward(self, x, t, clip_embed, volume_feats, x_concat, is_train=False):
        ...
        # >>> [CorrAdapter MODIFIED BEGIN: adapt UNetWrapper.forward to tuple-returning diffusion model]
        # DepthWiseAttention now returns
        # (prediction, memory_corrs, memory_qks). Original SyncDreamer used:
        # pred = self.diffusion_model(x, t, clip_embed, source_dict=volume_feats)
        pred, _, _ = self.diffusion_model(x, t, clip_embed, source_dict=volume_feats)
        # <<< [CorrAdapter MODIFIED END: adapt UNetWrapper.forward to tuple-returning diffusion model]
        return pred

    # >>> [CorrAdapter MODIFIED BEGIN: classifier-free guidance with CorrAdapter memory]
    ...
    def predict_with_unconditional_scale(self, x, t, clip_embed, volume_feats, x_concat,
                                         unconditional_scale, index=-1,
                                         memory_corrs=None, memory_qks=None):
        x_ = torch.cat([x] * 2, 0)
        t_ = torch.cat([t] * 2, 0)
        ...
        # Original SyncDreamer used:
        # s, s_uc = self.diffusion_model(x_, t_, clip_embed_, source_dict=v_).chunk(2)
        s_s_uc, memory_corrs, memory_qks = self.diffusion_model(
            x_,
            t_,
            clip_embed_,
            source_dict=v_,
        ...
        s, s_uc = s_s_uc.chunk(2)
        s = s_uc + unconditional_scale * (s - s_uc)
        # Original SyncDreamer returned only:
        # return s
        return s, memory_corrs, memory_qks
    # <<< [CorrAdapter MODIFIED END: classifier-free guidance with CorrAdapter memory]
...
class SyncMultiviewDiffusion(pl.LightningModule):
    def __init__(self, unet_config, scheduler_config,
                 ...
                 # >>> [Release Cleanup MODIFIED BEGIN: remove private default CLIP checkpoint path]
                 # Original SyncDreamer used a private absolute checkpoint path here.
                 clip_image_encoder_path="ckpt/ViT-L-14.pt",
                 # <<< [Release Cleanup MODIFIED END: remove private default CLIP checkpoint path]
...
class SyncDDIMSampler:
    ...
    @torch.no_grad()
    # >>> [CorrAdapter MODIFIED BEGIN: DDIM denoise step with CorrAdapter memory]
    # Original SyncDreamer signature:
    # def denoise_apply(self, x_target_noisy, input_info, clip_embed, time_steps,
    #                   index, unconditional_scale, batch_view_num=1, is_step0=False):
    def denoise_apply(self, x_target_noisy, input_info, clip_embed, time_steps,
                      index, unconditional_scale, batch_view_num=1, is_step0=False,
                      memory_corrs=None, memory_qks=None):
        """
        ...
        for ni in range(0, N, batch_view_num):
            ...
            if unconditional_scale!=1.0:
                ...
                noise, memory_corrs, memory_qks = self.model.model.predict_with_unconditional_scale(
                    x_target_noisy_,
                    time_steps_,
                    clip_embed_,
                    volume_feats_,
        ...
        # Original SyncDreamer returned only:
        # return x_prev
        return x_prev, memory_corrs, memory_qks
    # <<< [CorrAdapter MODIFIED END: DDIM denoise step with CorrAdapter memory]
    ...
    def sample(self, input_info, clip_embed, unconditional_scale=1.0, log_every_t=50, batch_view_num=1):
        ...
        # >>> [CorrAdapter MODIFIED BEGIN: keep CorrAdapter memory across DDIM timesteps]
        # DDIM-level CorrAdapter memory shared across timesteps.
        memory_corrs = []
        memory_qks = []
        for i, step in enumerate(iterator):
            ...
            #     x_target_noisy, input_info, clip_embed, time_steps, index,
            #     unconditional_scale, batch_view_num=batch_view_num,
            #     is_step0=index==0)
            x_target_noisy, memory_corrs, memory_qks = self.denoise_apply(
                x_target_noisy,
                input_info,
                clip_embed,
                time_steps,
        ...
        # <<< [CorrAdapter MODIFIED END: keep CorrAdapter memory across DDIM timesteps]
