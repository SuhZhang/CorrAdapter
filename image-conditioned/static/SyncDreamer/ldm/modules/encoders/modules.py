...
class FaceIdClipEncoder(AbstractEncoder):
    def __init__(self):
        ...
        # >>> [Release Cleanup MODIFIED BEGIN] Avoid a private absolute model
        # path in public code. Set FACE_ID_CKPT if this encoder is used.
        # Original LDM path was a private absolute checkpoint path.
        face_id_ckpt = os.environ.get("FACE_ID_CKPT", "ckpt/model_ir_se50.pth")
        self.id = FrozenFaceEncoder(face_id_ckpt, augment=True)
        # <<< [Release Cleanup MODIFIED END]
...
class FrozenT5Embedder(AbstractEncoder):
    ...
    def __init__(self, version="google/t5-v1_1-large", device="cuda", max_length=77):  # others are google/t5-v1_1-xl and google/t5-v1_1-xxl
        super().__init__()
        # >>> [Release Cleanup MODIFIED BEGIN] Avoid private HuggingFace cache
        # paths in public code. Use HF_HOME/SYNCDREAMER_HF_CACHE_DIR if needed.
        # Original LDM cache_dir was a private absolute cache path.
        hf_cache_dir = os.environ.get("SYNCDREAMER_HF_CACHE_DIR") or os.environ.get("HF_HOME")
        self.tokenizer = T5Tokenizer.from_pretrained(version, cache_dir=hf_cache_dir)
        self.transformer = T5EncoderModel.from_pretrained(version, cache_dir=hf_cache_dir)
        # <<< [Release Cleanup MODIFIED END]
        self.device = device
...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self, version="openai/clip-vit-large-patch14", device="cuda", max_length=77):  # clip-vit-base-patch32
        ...
        # Original LDM cache_dir was a private absolute cache path.
        hf_cache_dir = os.environ.get("SYNCDREAMER_HF_CACHE_DIR") or os.environ.get("HF_HOME")
        self.tokenizer = CLIPTokenizer.from_pretrained(version, cache_dir=hf_cache_dir)
        self.transformer = CLIPTextModel.from_pretrained(version, cache_dir=hf_cache_dir)
        # <<< [Release Cleanup MODIFIED END]
        self.device = device
        self.max_length = max_length   # TODO: typical value?
        self.freeze()
...
if __name__ == "__main__":
    ...
    sentences = ["a hedgehog drinking a whiskey", "the moon has risen", "A sentence with many punctuation marks: ?! : 'xx-y/@s'"]
