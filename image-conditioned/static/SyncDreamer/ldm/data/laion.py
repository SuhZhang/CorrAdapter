...
class DataWithWings(torch.utils.data.IterableDataset):
    def __init__(self, min_size, transform=None, target_transform=None):
        ...
        # >>> [Release Cleanup MODIFIED BEGIN] Avoid private absolute dataset
        # paths in public code. Configure these with environment variables when
        # this legacy dataset class is used.
        # Original LDM path values were private absolute dataset paths.
        watermark_kv = os.environ.get('LAION_WATERMARK_KV', 'laion5B-watermark-safety-ordered')
        aesthetic_kv = os.environ.get('LAION_AESTHETIC_KV', 'laion5B-aesthetic-tags-kv')
        self.kv = OnDiskKV(file=watermark_kv, key_format='q', value_format='ee')
        self.kv_aesthetic = OnDiskKV(file=aesthetic_kv, key_format='q', value_format='e')
