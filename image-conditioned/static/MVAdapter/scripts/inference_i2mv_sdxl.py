...
def prepare_pipeline(
    ...
    # >>> [CorrAdapter ADDED BEGIN: selectable adapter weight file]
    # Original MVAdapter always loaded mvadapter_i2mv_sdxl.safetensors. The
    # release script can also load a trained CorrAdapter checkpoint saved as a
    # sharded pytorch_model.bin.index.json directory.
    if adapter_weight_name == "auto":
        ...
        adapter_weight_name = next(
            (
                name
                for name in candidates
                if os.path.isfile(os.path.join(adapter_path, name))
            ),
            "mvadapter_i2mv_sdxl.safetensors",
        )
    # <<< [CorrAdapter ADDED END: selectable adapter weight file]
    pipe.load_custom_adapter(
        # >>> [CorrAdapter MODIFIED BEGIN: load selected adapter weight]
        # Original:
        #     adapter_path, weight_name="mvadapter_i2mv_sdxl.safetensors"
        adapter_path, weight_name=adapter_weight_name
        # <<< [CorrAdapter MODIFIED END: load selected adapter weight]
    )
...
def run_pipeline(
    ...
    device="cuda",
    azimuth_deg=None,
    # >>> [CorrAdapter MODIFIED BEGIN: configurable elevation for NVS evaluation]
    # Original MVAdapter used a fixed zero elevation:
    #     elevation_deg=[0] * num_views
    elevation_deg=0.0,
    # <<< [CorrAdapter MODIFIED END: configurable elevation for NVS evaluation]
):
    # Prepare cameras
    if azimuth_deg is None:
        azimuth_deg = [0, 45, 90, 180, 270, 315]
    cameras = get_orthogonal_camera(
        # >>> [CorrAdapter MODIFIED BEGIN: use the requested evaluation elevation]
        # Original:
        #     elevation_deg=[0] * num_views,
        elevation_deg=[float(elevation_deg)] * num_views,
        # <<< [CorrAdapter MODIFIED END: use the requested evaluation elevation]
...
if __name__ == "__main__":
    ...
    # >>> [CorrAdapter ADDED BEGIN: expose trained checkpoint weight name]
    parser.add_argument(
        "--adapter_weight_name",
        type=str,
        default="mvadapter_i2mv_sdxl.safetensors",
        help="Adapter file to load. Use 'auto' for local folders with a sharded trained checkpoint.",
    )
    # <<< [CorrAdapter ADDED END: expose trained checkpoint weight name]
    # Device
    parser.add_argument("--device", type=str, default="cuda")
    # Inference
    parser.add_argument("--num_views", type=int, default=6)  # not used
    # >>> [CorrAdapter MODIFIED BEGIN: expose NVS evaluation camera and size controls]
    # Original only accepted integer azimuths and hard-coded 768x768 inference.
    parser.add_argument(
        "--azimuth_deg", type=float, nargs="+", default=[0, 45, 90, 180, 270, 315]
    )
    parser.add_argument("--height", type=int, default=768)
    parser.add_argument("--width", type=int, default=768)
    parser.add_argument("--elevation_deg", type=float, default=0.0)
    # <<< [CorrAdapter MODIFIED END: expose NVS evaluation camera and size controls]
    ...
    images, reference_image = run_pipeline(
        ...
        text=args.text,
        image=args.image,
        # >>> [CorrAdapter MODIFIED BEGIN: use CLI-controlled resolution]
        # Original:
        #     height=768,
        #     width=768,
        height=args.height,
        width=args.width,
        # <<< [CorrAdapter MODIFIED END: use CLI-controlled resolution]
        ...
        # >>> [CorrAdapter MODIFIED BEGIN: forward evaluation elevation]
        elevation_deg=args.elevation_deg,
        # <<< [CorrAdapter MODIFIED END: forward evaluation elevation]
    )
    make_image_grid(images, rows=1).save(args.output)
    reference_image.save(args.output.rsplit(".", 1)[0] + "_reference.png")

    # >>> [CorrAdapter ADDED BEGIN: save one image per target view for metrics]
    # Original MVAdapter saved only the grid and reference image. The evaluator
    # reads 000.png, 001.png, ... directly.
    out_dir = os.path.dirname(args.output) or "."
    os.makedirs(out_dir, exist_ok=True)
    for i, im in enumerate(images):
        if getattr(im, "size", None) != (args.width, args.height):
            im = im.resize((args.width, args.height))
        im.save(os.path.join(out_dir, f"{i:03}.png"))
    # <<< [CorrAdapter ADDED END: save one image per target view for metrics]
