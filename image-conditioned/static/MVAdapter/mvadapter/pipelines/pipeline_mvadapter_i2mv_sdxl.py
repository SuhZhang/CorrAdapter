...
class MVAdapterI2MVSDXLPipeline(StableDiffusionXLPipeline, CustomAdapterMixin):
        ...
        **kwargs,
        ...
        Args:
            ...
            eta (`float`, *optional*, defaults to 0.0):
                Corresponds to the eta parameter in the DDIM paper: https://arxiv.org/abs/2010.02502. Only applies to
            ...
            guidance_rescale (`float`, *optional*, defaults to 0.0):
                ...
                Flawed](https://arxiv.org/pdf/2305.08891.pdf) `guidance_scale` is defined as `phi` in equation 16 of
        ...
        # >>> [CorrAdapter MODIFIED BEGIN: align prompt dtype with CorrAdapter UNet modules]
        # Original MVAdapter used prompt_embeds.dtype when preparing latents and
        # reference latents. CorrAdapter may add trainable local modules whose
        # dtype follows the UNet, so we explicitly align prompt-side tensors.
        target_dtype = next(self.unet.parameters()).dtype
        prompt_embeds = prompt_embeds.to(target_dtype)
        # <<< [CorrAdapter MODIFIED END: align prompt dtype with CorrAdapter UNet modules]
        ...
        latents = self.prepare_latents(
            ...
            height,
            width,
            # >>> [CorrAdapter MODIFIED BEGIN: use UNet dtype for denoising latents]
            # Original:
            #     prompt_embeds.dtype,
            target_dtype,
            # <<< [CorrAdapter MODIFIED END: use UNet dtype for denoising latents]
            device,
            generator,
            latents,
        )
        ...
        # >>> [CorrAdapter MODIFIED BEGIN: align pooled prompt dtype]
        add_text_embeds = add_text_embeds.to(target_dtype)
        # <<< [CorrAdapter MODIFIED END: align pooled prompt dtype]
        ...
        reference_latents = self.prepare_image_latents(
            ...
            batch_size,
            1,
            # >>> [CorrAdapter MODIFIED BEGIN: use UNet dtype for reference latents]
            # Original:
            #     prompt_embeds.dtype,
            target_dtype,
            # <<< [CorrAdapter MODIFIED END: use UNet dtype for reference latents]
            device,
            generator,
            add_noise=False,
        ) # BCHW, 1 4 96 96
        ...
        with torch.no_grad():
            ...
            self.unet(
                ...
                cross_attention_kwargs={
                    ...
                    "use_mv": False,
                    "use_ref": False,
                    # >>> [CorrAdapter ADDED BEGIN: initialize deterministic CorrAdapter step index]
                    # Original cross_attention_kwargs did not include index.
                    "index": 0,
                    # <<< [CorrAdapter ADDED END: initialize deterministic CorrAdapter step index]
                },
                return_dict=False,
            )
            ref_hidden_states = {
        ...
        # >>> [CorrAdapter ADDED BEGIN: select the highest attention up block]
        # Original MVAdapter passed the same cross_attention_kwargs at every
        # denoising step. CorrAdapter also needs the active up-block index.
        ...
        # <<< [CorrAdapter ADDED END: select the highest attention up block]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # >>> [CorrAdapter MODIFIED BEGIN: pass CorrAdapter runtime controls]
                # Original:
                #     cross_attention_kwargs=cross_attention_kwargs,
                #
                # CorrAdapter uses the denoising step index plus local matching
                # hyperparameters to build row-wise correspondences.
                cross_attention_kwargs_step = {
                    **cross_attention_kwargs,
                    "index": int(len(timesteps) - 1 - i),
                    "local_radius": 3,
                    "local_tau": 1.0,
                    "local_conf_thresh": 0.05, # 0.2,
                    # apply local matching at the highest up block that has attention
                    "local_up_block_index": highest_up_attn_index,
                }
                # <<< [CorrAdapter MODIFIED END: pass CorrAdapter runtime controls]
                noise_pred = self.unet(
                    latent_model_input,
                    t,
                    encoder_hidden_states=prompt_embeds,
                    timestep_cond=timestep_cond,
                    cross_attention_kwargs=cross_attention_kwargs_step,
    ...
    def _load_custom_adapter(self, state_dict):
        # >>> [CorrAdapter MODIFIED BEGIN: accept adapter-only or full training checkpoints]
        # Original:
        #     self.unet.load_state_dict(state_dict, strict=False)
        #
        # The training release path can produce either adapter-only keys
        # ("...processor.to_q_mv.weight") or full-module keys prefixed by
        # "unet." / "cond_encoder.". Normalize both formats here.
        if any(k.startswith("unet.") for k in state_dict.keys()):
            unet_state_dict = {
                k[len("unet.") :]: v for k, v in state_dict.items() if k.startswith("unet.")
            }
        else:
            unet_state_dict = state_dict
        self.unet.load_state_dict(unet_state_dict, strict=False)
        # <<< [CorrAdapter MODIFIED END: accept adapter-only or full training checkpoints]
        # >>> [CorrAdapter ADDED BEGIN: initialize local branch from MV weights]
        # Original MVAdapter checkpoints do not contain to_q_loc/to_k_loc/to_v_loc.
        # When such keys are absent, initialize the CorrAdapter local branch from
        # the MV branch so training-free inference and fine-tuning both start
        # from the original adapter behavior.
        policy = os.environ.get("MVADAPTER_LOC_LOAD_POLICY", "prefer_loc").lower()
        force_mv = policy == "force_mv"
        has_loc_keys = any(
            ("to_q_loc" in k) or ("to_k_loc" in k) or ("to_v_loc" in k)
            for k in state_dict.keys()
        )
        if force_mv or (not has_loc_keys):
            with torch.no_grad():
                for name, module in self.unet.named_modules():
                    if hasattr(module, "to_q_loc") and hasattr(module, "to_q_mv"):
                        module.to_q_loc.weight.copy_(module.to_q_mv.weight)
                    if hasattr(module, "to_k_loc") and hasattr(module, "to_k_mv"):
                        module.to_k_loc.weight.copy_(module.to_k_mv.weight)
                    if hasattr(module, "to_v_loc") and hasattr(module, "to_v_mv"):
                        module.to_v_loc.weight.copy_(module.to_v_mv.weight)
        # <<< [CorrAdapter ADDED END: initialize local branch from MV weights]
        # >>> [CorrAdapter MODIFIED BEGIN: load condition encoder from prefixed checkpoint]
        # Original:
        #     self.cond_encoder.load_state_dict(state_dict, strict=False)
        if any(k.startswith("cond_encoder.") for k in state_dict.keys()):
            cond_state_dict = {
                k[len("cond_encoder.") :]: v
                for k, v in state_dict.items()
                if k.startswith("cond_encoder.")
            }
        else:
            cond_state_dict = state_dict
        self.cond_encoder.load_state_dict(cond_state_dict, strict=False)
        # <<< [CorrAdapter MODIFIED END: load condition encoder from prefixed checkpoint]
